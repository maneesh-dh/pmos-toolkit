...
| P10 | Split T43 (atomic version bump) into T43a (stage diff in working tree, no commit) + T43 (release commit gated on T39–T41 passing green) | (a) Split into stage + release (chosen), (b) Single T43 with explicit "Depends on: T39, T40, T41" gate, (c) Keep monolithic | Loop-2 blind-reviewer surfaced that the original single-task T43 lands the version bump before manual integration tests run; once 2.24.0 commits, all real users get v2 even if T39–T41 fail. (a) makes the gate explicit and reversible (the staged diff in T43a can be `git checkout --` if integration fails). (b) merges the gate into a single task but loses the staging step (the version diff would still need to exist somewhere for fresh-session integration runs to load v2 SKILL content). (c) carries the original risk. Trade-off: 2 tasks instead of 1, but resolves R6 (version bump before integration verified) cleanly |
| P11 | /execute v2 (T35) parses suffixed task IDs (`T26a`, `T29c`, `T43a`) AS strings; per-task log files use `task-<N><suffix>.md` (`task-26a.md`); frontmatter `task_number` accepts string when a suffix is present, integer otherwise | (a) Suffix-aware parsing in T35 (chosen), (b) Renumber all 48 tasks to integer-only T1–T48, (c) Defer until /execute runs and break it then | Phase 5 finding: Loop-1 task splits (T26→T26a/b, T29→T29a/b/c, T31→T31a/b, T43→T43a) introduced suffixed IDs that conflict with execute/SKILL.md's `task_number: <integer>` schema. (a) is a small extension to T35's parsing scope (regex `T([0-9]+)([a-z]?)`) and keeps Loop-1's split-history visibility. (b) requires ~30 cross-reference edits across the plan body and loses the split lineage. (c) ships a known defect. Decision recorded so T35 implementation explicitly handles suffixes |

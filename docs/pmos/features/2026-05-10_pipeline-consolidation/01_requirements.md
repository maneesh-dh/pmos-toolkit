...
- Tier gate: Tier 3 mandatory; Tier 2 soft with Recommended=Run; Tier 1 soft with Recommended=Skip.
...
- Phase 2 dispatches one subagent per transcript jsonl (parallel where possible). Each subagent extracts only `(skill-invocation, ±N surrounding turns)` slices, returns a structured findings list per the existing /retro output schema. Parent aggregates without re-reading transcripts.
- Aggregation: `(skill, severity, first-100-chars-of-finding)` hash for fuzzy dedup; `(skill, finding-hash)` for count + sessions-seen-in.
...
|---|---|---|
| No msf-req-findings.md at /verify time on a Tier-3 feature | folded MSF-req failed silently OR was skipped at Tier-1/2 | `/verify` checks tier from `02_spec.md` frontmatter; if Tier 3 and findings missing → blocking. If Tier 1/2 → advisory only |
...
| D2 | MSF folding triggers per Tier — Tier 3 mandatory (no gate), Tier 1/2 soft gate | (a) Always-mandatory at all tiers (b) Always-soft-gated at all tiers (c) Tier-keyed mandatoriness | (c) — Tier 1 (bug fix) does not warrant MSF rigor; Tier 3 (feature) does; Tier 2 should be opt-in with Recommended=Run. Eliminates the gate-fatigue at Tier 3 while preserving lightweight Tier 1 |
